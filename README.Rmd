...
## Contributions
The author of this package is Jeremy Chan. The example data is derived from the Cryo-ET Data Portal by CZ Biohub submitted by Ke et al. (2018). The denoising algorithms are implemented using the `EBImage` package. The segmentation algorithms are implemented using the `ijtiff` and `autothresholdr` packages. Visualisation is done using base R plotting functions and `ggplot2`. Generative AI was used for proofreading, code optimisation, and assistance in writing documentation.
