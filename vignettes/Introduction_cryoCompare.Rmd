...
We can also call `runSegmmentation` with a ground truth mask to evaluate segmentation accuracy:
```
runSegmentation(image = TS_001.133,
                        methods = c("Otsu", "Triangle"),
                        ground_truth = TS_001.133_ground_truth)
```
This will output the segmentation results, along with the Dice Coefficient accuracy metric for each method visualised using a plot.
