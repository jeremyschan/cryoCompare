...
Next, we will run the `runPipeline` function on the sample tomogram using Huang and Mean segmentation methods (remember to put them into a vector!):
```
...
```
This will allow us to see how well each segmentation method performed against the ground truth data. This will return a list containing the segmentation results as well as the Dice Coefficient accuracy metrics for each method, through a table with exact values and also visualised via a plot.

If you only want to run one of the functions within the pipeline, you can choose which one to use. For example, we can run just the denoising step using the `runDenoising` function:
```
runDenoising(image = TS_001.133)
```
This will prompt you to enter values for the Gaussian sigma and Median filter size, then display the denoised images for each method. The function will return a list containing the denoised images for each method as well as save them to the data folder of the package by default.
...
runSegmentation(image = TS_001.133,
                        methods = c("Huang", "Mean"))
```
These return values are the same as `runPipeline`, and will display the segmentation results for each method and return a list containing the segmentation masks and threshold values for each method.

We can also call `runSegmentation` with a ground truth mask to evaluate segmentation accuracy:
```
runSegmentation(image = TS_001.133,
                        methods = c("Otsu", "Triangle"),
                        ground_truth = TS_001.133_ground_truth)
```
As mentioned, this will output the segmentation results, along with the Dice Coefficient accuracy metric for each method visualised using a plot.

We can also separately visualise the segmentation results using the `viewSegmentation` function, which will display the segmented images for the specified methods:
```
viewSegmentation(image = TS_001.133,
                         methods = c("Huang", "Mean"))
```
This will display the segmented images for each method specified.

Lastly, we can perform threshold-based segmentation using the `thresholdSegmentation` function, returning a threshold value and a binary mask for each method:
```
...
```
This will return the threshold value used for segmentation as well as display the binary mask generated using the specified method.

## Package References

- Chan, J. (2025). cryoCompare. Unpublished. https://github.com/jeremyschan/cryoCompare
